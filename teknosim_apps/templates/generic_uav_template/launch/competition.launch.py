...
def generate_launch_description():
    pkg_share = get_package_share_directory('generic_uav_template')
    ...
    velocity_controller_spawner = Node(
        package='controller_manager',
        executable='spawner',
        arguments=['uav_velocity_controller', '--controller-manager', '/controller_manager'],
        output='screen'
    )

    return LaunchDescription([
        mode_arg,
        LogInfo(msg=f"[TeknoSim Generic UAV] Selected Operation Mode: {teknosim_mode}"),
        controller_manager,
